...
function Map() 
{
	...
	$request = $smcFunc['db_query']('', '
		...
		WHERE latitude IS NOT NULL AND longitude IS NOT NULL AND latitude <> 0.0 AND longitude <> 0.0'
	);
...
function MapsXML() 
{
	...
	$request = $smcFunc['db_query']('', $query,	array('last_week' => $last_week));
	...
	if (isset($memberContext)) 
	{
		...
		foreach ($memberContext as $marker) 
		{
			...
			<table class="googleMaps" border="0">
				<tr>
					<td style="white-space: nowrap;" align="left">
						<a href="' . $marker['online']['href'] . '"><img src="' . $marker['online']['image_href'] . '" height="11" alt="' . $marker['online']['text'] . '" class="avatar" border="0" /></a>
			...
			if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']) && !empty($marker['avatar']['image']))
				$datablurb.= '<td style="height:100px;width:100px" rowspan="3">' . $marker['avatar']['image'] . '</td>';
			$datablurb.= '</tr><tr><td>'; 
			
			// Show the post group if and only if they have no other group or the option is on, and they are in a post group.
			if ((empty($settings['hide_post_group']) || $marker['group'] == '') && $marker['post_group'] != '') 
				$datablurb .= $marker['post_group'] . '<br />';
				
			$datablurb .= $marker['group_stars'] . '</td>';
			$datablurb .= '</tr><tr>
					<td><font color="#000000">' . $marker['title'] . '</font></td>
				</tr><tr>
			...
			if ($marker['website']['url'] != '' && !isset($context['disabled_fields']['website'])) 
				$datablurb.= '<a href="' . $marker['website']['url'] . '" title="' . $marker['website']['title'] . '" target="_blank" class="new_win">' . ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/www_sm.gif" height="16" alt="' . $marker['website']['title'] . '" border="0" />' : $txt['www']) . '</a>&nbsp;';

			// Don't show the email address if they want it hidden.
			if (in_array($marker['show_email'], array('yes', 'yes_permission_override', 'no_through_forum'))) 
				$datablurb.= '<a href="' . $scripturl . '?action=emailuser;sa=email;uid=' . $marker['id'] . '">' . ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/email_sm.gif" alt="' . $txt['email'] . '" height="16" title="' . $txt['email'] . '" />' : $txt['email']) . '</a>&nbsp;';

			// Show the PM tag
			$datablurb.= '<a href="' . $scripturl . '?action=pm;sa=send;u=' . $marker['id'] . '">';
